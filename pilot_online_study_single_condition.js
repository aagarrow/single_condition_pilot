/******************************************** 
 * Pilot_Online_Study_Single_Condition *
 ********************************************/
...
let expName = 'pilot_online_study_single_condition';  // from the Builder filename that created this script
let expInfo = {
    '': '',
};
...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'sounds/HugginsPitch_calibration.flac', 'path': 'sounds/HugginsPitch_calibration.flac'},
    {'name': 'sounds/HugginsPitch_example_2.flac', 'path': 'sounds/HugginsPitch_example_2.flac'},
  ]
});
...
async function experimentInit() {
  ...
  // Function to fetch participant data and handle it
  async function fetchParticipantData() {
    try {
      // Fetch the data from the Google Sheet
      // const response = await fetch('https://script.google.com/macros/s/AKfycbz0WpfxnAJEwkoBglOkEVdudO1JOKHqIRSIsWiv61cd0Mwqq9lmaBCFUSylCp3b8IPIaA/exec'); // Old script
      const response = await fetch('https://script.google.com/macros/s/AKfycbwAe35tNwKbatT2qVle-D3ifzgUtwV0JnHaTNA98wOn3cn6k9SVcURAyKCym1wHPrQY/exec');
      const data = await response.json();
      
      // Process the fetched data to zero-pad the participant number
      ...
      // Set the experiment's participant information
      ...
      // Now proceed with the experiment
      experimentComplete = false;
  
      const melodies_count = [];
  
      for (let i = 0; i < num_exp_melodies; i++) {
          let paddedNumber = i.toString().padStart(3, '0');
          let suffix = `exposure_melody_${paddedNumber.toString()}.wav`;
          melodies_count.push(suffix);
      };
  
      for (let i = 0; i < num_correct_melodies; i++) { // Number of targets in test phase
          let paddedNumber = i.toString().padStart(3, '0');
          let suffix = `target_melody_${paddedNumber.toString()}.wav`;
          melodies_count.push(suffix);
      };
  
      for (let i = 0; i < num_foil_melodies; i++) {
          let paddedNumber = i.toString().padStart(3, '0');
          let suffix = `foil_melody_${paddedNumber.toString()}.wav`;
          melodies_count.push(suffix);
      };
  
      const prefix = `sounds/participant_${expInfo['participant'].toString()}_`;
  
      const melodies_list = melodies_count.map(melody => prefix + melody);
  
      const conditions_list = [`conditions/exp_stims_${expInfo['participant'].toString()}.csv`,`conditions/test_stims_${expInfo['participant'].toString()}.csv`];
  
      const resources_list = [];
  
      for (let i = 0; i < melodies_list.length; i++) {
          resources_list.push(melodies_list[i]);
      };
  
      for (let i = 0; i < conditions_list.length; i++) {
          resources_list.push(conditions_list[i]);
      };
  
      const stimuli = [];
  
      // for loop (to format resources_list to be searchable by prepareResources function)
      for (let i = 0; i < resources_list.length; i++) {
          let stimulus = {
              name: resources_list[i],
              path: resources_list[i]
          };
          stimuli.push(stimulus);
      };
  
      const test_tones_list = [`sounds/participant_${expInfo['participant'].toString()}_low_freq_vol_adj.wav`,`sounds/participant_${expInfo['participant'].toString()}_high_freq_vol_adj.wav`];
  
      for (let i = 0; i < test_tones_list.length; i++) {
          stimuli.push(test_tones_list[i]);
      };
  
      const attention_tones_list = [
      `sounds/participant_${expInfo['participant'].toString()}_1_clicks_audio.wav`,
      `sounds/participant_${expInfo['participant'].toString()}_2_clicks_audio.wav`,
      `sounds/participant_${expInfo['participant'].toString()}_3_clicks_audio.wav`,
      `sounds/participant_${expInfo['participant'].toString()}_4_clicks_audio.wav`,
      `sounds/participant_${expInfo['participant'].toString()}_5_clicks_audio.wav`
      ];
  
      for (let i = 0; i < attention_tones_list.length; i++) {
          stimuli.push(attention_tones_list[i]);
      };
  
      const huggins_tones_list = [
      `sounds/HugginsPitch_calibration.flac`,
      `sounds/HugginsPitch_example_2.flac`,
      `sounds/HugginsPitch_set1_1.flac`,
      `sounds/HugginsPitch_set1_2.flac`,
      `sounds/HugginsPitch_set1_3.flac`,
      `sounds/HugginsPitch_set2_1.flac`,
      `sounds/HugginsPitch_set2_2.flac`,
      `sounds/HugginsPitch_set2_3.flac`,
      `sounds/HugginsPitch_set3_1.flac`,
      `sounds/HugginsPitch_set3_2.flac`,
      `sounds/HugginsPitch_set3_3.flac`,
      `sounds/HugginsPitch_set4_1.flac`,
      `sounds/HugginsPitch_set4_2.flac`,
      `sounds/HugginsPitch_set4_3.flac`,
      `sounds/HugginsPitch_set5_1.flac`,
      `sounds/HugginsPitch_set5_2.flac`,
      `sounds/HugginsPitch_set5_3.flac`,
      `sounds/HugginsPitch_set6_1.flac`,
      `sounds/HugginsPitch_set6_2.flac`,
      `sounds/HugginsPitch_set6_3.flac`,
      ];
  
      for (let i = 0; i < huggins_tones_list.length; i++) {
          stimuli.push(huggins_tones_list[i]);
      };
  ...
  // Start the asynchronous fetch operation (to assign participant number)
  ...
  text_welcomeMessage = new visual.TextStim({
    ...
    text: "You will now experience two blocks of listening: In Block 1, you will simply click through the test and listen to each melody when it plays. In Block 2, you will listen to each melody and choose an answer that best represents the current melody compared to the melodies in Block 1.\n\nIn both blocks, a small number of attention checks will be carried out in which you will hear a short audio clip containing 'beeps' and then be asked how many beeps were in the clip.\n\nPlease click 'Next' to proceed.",
    font: 'Arial',
    units: undefined, 
  ...
  text_preTestMessage = new visual.TextStim({
    ...
    text: 'You have now completed Block 1.\n\nBlock 2 will begin when you click the button below. Please listen to all the melodies carefully and when answering each question, respond based on the melodic sequence you just heard compared to the sequences you heard in Block 1.',
    font: 'Arial',
    units: undefined, 
    pos: [0, 0.1], height: 0.05,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
...
function headphone_calibrateRoutineBegin(snapshot) {
  return async function () {
    ...
    sound_headphoneCalibrate.setValue('sounds/HugginsPitch_calibration.flac');
    sound_headphoneCalibrate.setVolume(1.0);
    // keep track of which components have finished
...
function headphone_exampleRoutineBegin(snapshot) {
  return async function () {
    ...
    sound_headphoneExample.setValue('sounds/HugginsPitch_example_2.flac');
    sound_headphoneExample.setVolume(1.0);
...
function trials_expAudioLoopBegin(trials_expAudioLoopScheduler, snapshot) {
  return async function() {
    ...
    trials_expAudio = new TrialHandler({
      ...
      extraInfo: expInfo, originPath: undefined,
      trialList: (("conditions/exp_stims_" + expInfo["participant"].toString()) + ".csv"),
...
function trials_testAudioLoopBegin(trials_testAudioLoopScheduler, snapshot) {
  return async function() {
    ...
    trials_testAudio = new TrialHandler({
      ...
      nReps: 1, method: TrialHandler.Method.RANDOM,
      extraInfo: expInfo, originPath: undefined,
      trialList: (("conditions/test_stims_" + expInfo["participant"].toString()) + ".csv"),
...
function headphone_checkRoutineBegin(snapshot) {
  return async function () {
    ...
    audioFilePath = 'sounds/HugginsPitch_set' + huggins_trial_counter + '_' + correct_file + '.flac';
    sound_huggins.setValue(audioFilePath);
    sound_huggins.setVolume(1.0);
    // keep track of which components have finished
...
function volume_adjustRoutineBegin(snapshot) {
  return async function () {
    ...
    sound_highFreq.setValue((("sounds/participant_" + expInfo["participant"].toString()) + "_high_freq_vol_adj.wav"));
    sound_highFreq.setVolume(1.0);
    sound_lowFreq.setValue((("sounds/participant_" + expInfo["participant"].toString()) + "_low_freq_vol_adj.wav"));
    sound_lowFreq.setVolume(1.0);
...
function exp_attention_playbackRoutineBegin(snapshot) {
  return async function () {
    ...
    audioFilePath = 'sounds/participant_' + expInfo['participant'] + '_'+ curr_click_num + '_clicks_audio.wav';
